...
int op_start=0;
double percentage;
double percentage1; //used for cheat function
int currency;
int bet;
int highscore;
...
public:
    ...
    double cards_left=0,favorable_cards=0,cards_left1=0,favorable_cards1=0; //we use those for percentage of winning
    ...
    void DisplayCards()
    {
        ...
        printf("PRESS: 1 - DRAW CARD, 2 - HOLD, 3 - DOUBLE DOWN           Bet:%i                                              9-Cheat?\n\n",bet);
    ...
    double Percentage() //here we define percentage function
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 13; j++)
            {
                ...
                {
                    cards_left++;
                    if (j+1<=21-op_start || (j>9 && op_start+10<=21))
                        favorable_cards++;
                }
            }
        }
    ...
    void Hold()  //what happens after we hold, computer start to draw it's card
    {
        ...
        while (op_start<=start)
        {
            ...
            usedCards[colorIndex][rankIndex]=Card_Order_Counter; //changing from 0 to a counted value so it's known the order of drawing
            Card_Order_Counter++;
            Add_op_Points();
            DisplayCards();
    ...
    void Cheat()
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 13; j++)
            {
                if (usedCards[i][j]==0) //check from the list of not drawn cards
                {
                    cards_left1++;
                    if (j+1<=21-start || (j>9 && start+10<=21))
                        favorable_cards1++;
                }
            }
        }
        percentage1=(favorable_cards1/cards_left1)*100;
        printf("Chance of success: %f% \n",percentage1);
        percentage1=0;
        favorable_cards1=0;
        cards_left1=0;
